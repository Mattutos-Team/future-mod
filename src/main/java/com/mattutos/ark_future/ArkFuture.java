...
//        context.registerConfig(ModConfig.Type.COMMON, Config.SPEC);
    ...
    private void addCreative(BuildCreativeModeTabContentsEvent event) {
        if (event.getTabKey() == CreativeModeTabs.INGREDIENTS) {
            event.accept(ItemInit.ANCIENT_ORE_ITEM);
            event.accept(ItemInit.ANCIENT_ORE_BLOCK_ITEM);
            event.accept(ItemInit.ANCIENT_ORE_INGOT_ITEM);
            event.accept(ItemInit.ANCIENT_ORE_VEIN_BLOCK_ITEM);
            event.accept(ItemInit.DEEPSLATE_ANCIENT_ORE_VEIN_BLOCK_ITEM);
        }
    }
}
